...
import {
  hasPlayedToday,
  getTodayGameResult,
} from "@/app/utils/localGameStorage";
...
export default function Home() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-gray-900 to-black text-white px-4 ">
      ...
      {(
        user
          ? hasUserPlayedToday && firestoreTodayResult
          : hasLocalUserPlayedToday && todayResult
      ) ? (
        <div className="backdrop-blur-md bg-white/10 border border-gray-700 shadow-2xl rounded-2xl p-8 max-w-md w-full text-center flex flex-col items-center">
          ...
          <h2 className="text-2xl font-bold mb-4 text-blue-200 drop-shadow mt-8">
            Today's Result 🎯
          </h2>
          <div className="space-y-3 mb-6 w-full">
            ...
            <p className="text-gray-300 text-sm mt-2">
              {getScoreRating(
                user ? firestoreTodayResult.score : todayResult.score
              )}
            </p>
          </div>
          {/* <button
          ...
          {!user ? (
            <div className="backdrop-blur-md bg-white/10 border border-gray-700 rounded-2xl p-6 mb-6
             max-w-md w-full text-center shadow-lg mt-6 dark:bg-gray-500">
              <h3 className="text-xl font-bold mb-4 text-lime-300 drop-shadow">
                Want to save your progress?
              </h3>
              <p className="text-gray-300 mb-4">
                Create a free account to track your points and stats
              </p>
              <button
                onClick={() => signInWithGoogle()}
                disabled={authLoading}
                className="w-full bg-white text-black px-4 py-3 rounded font-semibold hover:bg-gray-100 transition flex items-center justify-center cursor-pointer hover:scale-105"
              >
                {authLoading ? (
        ...
        <div className="space-y-6">
          <div className="flex flex-col items-center gap-4">
            <button
              ...
              className="bg-lime-500 hover:bg-lime-600 text-black font-bold py-4 px-12 rounded-full text-2xl shadow-lg transition-all duration-300 hover:scale-105 cursor-pointer"
            >
              🎮 PLAY
            </button>
            {user ? (
              <button
                onClick={handleSignOut}
                className="bg-gray-700 hover:bg-gray-800 text-white font-semibold py-3 px-8 rounded-full text-lg shadow transition-all duration-300 hover:scale-105 cursor-pointer"
              >
                Log Out
              </button>
            ) : (
